...
describe('HOTEL CLASS TESTS', function() {
  ...
  it('Should store all of the bookings in an array', () => {
    expect(hotel.bookings).to.deep.equal([
      bookings[0], bookings[1], bookings[2], bookings[3], bookings[4], bookings[5]
    ]);
  });
  ...
  it('Should have an attribute to store available rooms that is an empty array by default', () => {
  ...
  it('Given a userID, should update the user\'s bookings with the rooms they\'ve booked', () => {
    ...
    expect(hotel.customers[0].bookings).to.deep.equal([
      bookings[0], bookings[1]
    ]);
    expect(hotel.customers[2].bookings).to.deep.equal([
      bookings[4], bookings[5]
    ]);
  });

  it('Given a userID, should calculate their spending for the rooms they\'ve booked past/present/future', () => {
    hotel.assignUsersBookings(1)
    hotel.assignUsersBookings(3)
    expect(hotel.calculateUserSpending(1)).to.equal(
      Math.round((358.4 + 429.44) * 100) / 100
    );
    expect(hotel.calculateUserSpending(3)).to.equal(
      Math.round((491.14 + 429.44) * 100) / 100
    );
  });

  it('Given a valid username and password, should return true', () => {
